...
public class PieChartView extends View implements Runnable {
	...
	// 默认颜色
	private static final String[] DEFAULT_ITEMS_COLORS = { "#20c8b1",
			"#92c9c3", "#a8efe9", "#c2e9ee", "#71cf97", "#d6d6b4", "#d7b97b",
			"#eed0b4", "#f2afa6", "#fb6571", "#0000FF", "#363636", "#FFDAB9",
			"#90EE90", "#8B008B", "#00BFFF", "#00FF00", "#006400", "#00FFFF",
			"#00FFFF", "#668B8B", "#000080", "#008B8B" };
	...
	public void setItemsSizes(float[] items) {
		if (items != null && items.length > 0) {
			...
			this.itemSizesTemp = new float[]{(float) 100.0};
			reSetTotal();
			refreshItemsAngs();
			setItemsColors(new String[]  { "#20c8b1"});
		}
		invalidate();
	}
	 ...
	 * @param total2
	 */
	public void setActualTotal(double total2) {
		this.totalNum = total2;
	}
		...
		if (itemsAngle != null && itemsBeginAngle != null) {
			...
			for (int i = 0; i < itemsAngle.length; i++) {
				if (itemPostion == i && !isRotating) {
					...
					case TO_TOP:
						// 向上移动
						oval = new RectF(leftTop, leftTop - separateDistence,
								rigthBottom, rigthBottom);
						break;
					case TO_BOTTOM:
						// 向下移动
						oval = new RectF(leftTop, leftTop, rigthBottom,
					...
					case TO_LEFT:
						// 向左移动
						oval = new RectF(leftTop - separateDistence, leftTop,
								rigthBottom, rigthBottom);
						break;
					default:
						break;
			...
			canvas.drawCircle(
					...
					radius
							...
							+ context.getResources().getDimension(
									R.dimen.statistics_circle_radius), p1);
			p1.setColor(Color.WHITE);
			canvas.drawCircle(centerX, centerY, radius / 2, p1);
	...
	private void drawText(Canvas canvas, float x, float y, int radius,
		...
		showInfo = "本月" + category + "金额一共 " + totalNum + " 元";
			...
			case MotionEvent.ACTION_DOWN:
				...
				if ((x1 - r) * (x1 - r) + (y1 - r) * (y1 - r) - r * r <= 0) {
					...
					if (bitmap != null && triggerX <= x1
							&& x1 <= triggerX + bitmap.getWidth()
							&& triggerY <= y1
							&& y1 <= triggerY + bitmap.getHeight()) {
						notifyTriggerClickListeners();
					} else if ((getWidth() / 2 - x1) * (getWidth() / 2 - x1)
						...
						// 在点击了扇形
						position = getShowItem(getTouchedPointAngle(r, r, x1,
								y1));
